...
fn normalize_raw_object_addr_rejects_inline_nanboxed_receivers() {
    ...
    // BIGINT: heap bigint headers are dereferenceable; the tag passes through.
    let bigint_addr: u64 = 0x0000_0003_0000_0000;
    let bigint = crate::value::BIGINT_TAG | bigint_addr;
    assert_eq!(
        normalize_raw_object_addr(bigint),
        bigint_addr as usize,
        "heap-bigint receiver must pass through unchanged",
    );
}
